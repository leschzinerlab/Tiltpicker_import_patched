...
def checkConflicts(params):
	...
	if not os.path.exists('/usr/bin/ApTiltPicker_import.py'):
	...
	if os.path.exists('/usr/bin/ApTiltPicker_import.py'):
		aptiltpath='/usr/bin/ApTiltPicker_import.py'
...
def runTiltPicker(params,aptiltpath):
	...
	while i<=totlines:
		...
		if os.path.exists('temp_aptiltout.spi'):
		...
		#Launch ApTiltPicker: 
		cmd='%s -l %s_temp_bin.mrc -r %s_temp_bin.mrc --picks1=%s.spi --picks2=%s.spi --output=temp_aptiltout.spi' %(aptiltpath,untiltmicro[:-4],tiltmicro[:-4],untiltbox[:-4],tiltbox[:-4])
		if params['debug'] is True:
			print cmd
		subprocess.Popen(cmd,shell=True).wait()

		if not os.path.exists('temp_aptiltout.spi'):
			print 'No particle picks for %s and %s. Continuing on to next micrograph.' %(untiltmicro,tiltmicro)
			i = i + 1
			continue

		if len(open('temp_aptiltout.spi','r').readlines()) < 40:
...
def parseApTilt(aptilt,box,binning,untiltout,tiltout,microdims,debug):
	...
	f=aptilt
	f1 = open(aptilt,'r')
	tot = (len(f1.readlines())-24)/2
	f1.close()

	leftmicro=linecache.getline(f,23).split()[4]
	rightmicro=linecache.getline(f,24+tot).split()[4]
